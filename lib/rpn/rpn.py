...
def assign_pyramid_anchor(feat_shapes, gt_boxes, im_info, cfg, feat_strides=(4, 8, 16, 32, 64),
    ...
    :param instance_index: other than this all gt boxes will be ignored when cfg is moderate strictness
    :return: dict of label
    'label': of shape (batch_size, 1) <- (batch_size, num_anchors, feat_height, feat_width)
    'bbox_target': of shape (batch_size, num_anchors * 4, feat_height, feat_width)
    'bbox_inside_weight': *todo* mark the assigned anchors
    ...
    if gt_boxes.size > 0:
        ...
        # overlaps (ex, gt)
        # try:
        #     if cfg.TRAIN.STRICTNESS == 'moderate' and instance_index:
        #         overlaps = bbox_overlaps(fpn_anchors.astype(np.float), gt_boxes[instance_index].astype(np.float))
        #     else:
        #         overlaps = bbox_overlaps(fpn_anchors.astype(np.float), gt_boxes.astype(np.float))
        #
        # # except NameError: # if cfgs dont have strictness
        # #     overlaps = bbox_overlaps(fpn_anchors.astype(np.float), gt_boxes.astype(np.float))
        # except AttributeError: # if cfgs dont have strictness
        overlaps = bbox_overlaps(fpn_anchors.astype(np.float), gt_boxes.astype(np.float))
        ...
        # try:
        #     if cfg.TRAIN.STRICTNESS == 'moderate':
        #         ignore_boxes = gt_boxes.copy()
        #         del ignore_boxes[instance_index]
        #         if ignore_boxes.size > 0:
        #             overlaps = bbox_overlaps(fpn_anchors.astype(np.float), ignore_boxes.astype(np.float))
        #             argmax_overlaps = overlaps.argmax(axis=1)
        #             ig_max_overlaps = overlaps[np.arange(len(fpn_anchors)), argmax_overlaps]
        #             gt_argmax_overlaps = overlaps.argmax(axis=0)
        #             gt_max_overlaps = overlaps[gt_argmax_overlaps, np.arange(overlaps.shape[1])]
        #             gt_ig_argmax_overlaps = np.where(overlaps == gt_max_overlaps)[0]
        #
        #             # fg label: for each gt, anchor with highest overlap
        #             fpn_labels[gt_ig_argmax_overlaps] = -1
        #             # fg label: above threshold IoU
        #             fpn_labels[ig_max_overlaps >= cfg.TRAIN.RPN_POSITIVE_OVERLAP] = -1
        #
        # except AttributeError: # if cfgs dont have strictness
        #     pass
    else:
    ...
    if balance_scale_bg:
        ...
        if len(bg_inds) > num_bg:  # lets make most of the background ignore, specifically all but #bg=cfg.256-#fg
            disable_inds = npr.choice(bg_inds, size=(len(bg_inds) - num_bg), replace=False)
            if DEBUG:
                disable_inds = bg_inds[:(len(bg_inds) - num_bg)]
            fpn_labels[disable_inds] = -1
